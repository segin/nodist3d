...
## 2026-05-04 - Accessibility in Scene Graph
**Learning:** Scene graph list items need `role="button"` and `tabindex="0"` for keyboard accessibility, plus `keydown` handlers. Icon-only buttons (timeline controls) need `aria-label`.
**Action:** Always verify keyboard navigation for interactive list items.
