...
const THREE = {
  ...
  // Loaders
  Loader: class { constructor(manager) { this.manager = manager; } load() {} parse() {} },
  FileLoader: class { constructor(manager) { this.manager = manager; } load() {} },
  TextureLoader: jest.fn().mockImplementation(() => ({
    load: jest.fn((url, onLoad) => { if (onLoad) onLoad(new THREE.Texture()); }),
  })),
  ObjectLoader: jest.fn().mockImplementation(() => ({
    parse: jest.fn((data, onLoad) => { if (onLoad) onLoad(new THREE.Scene()); }),
  })),
  FontLoader: jest.fn().mockImplementation(() => ({
    load: jest.fn((url, onLoad) => { if (onLoad) onLoad(new THREE.Font()); }),
  })),

  // Objects
  Texture: jest.fn(() => ({ dispose: jest.fn() })),
  Font: jest.fn(() => ({ type: 'Font' })),
  Scene: jest.fn(() => {
    const scene = {
      children: [],
      add: jest.fn((object) => {
  ...
  PointLight: jest.fn(() => ({ ...mockObject3D, isPointLight: true, color: { set: jest.fn() }, intensity: 1 })),
  AmbientLight: jest.fn(() => ({ ...mockObject3D, isAmbientLight: true })),

  // Helpers
  GridHelper: jest.fn(() => ({ ...mockObject3D, isGridHelper: true, toJSON: () => ({ object: { type: 'GridHelper' } }) })),
  AxesHelper: jest.fn(() => ({ ...mockObject3D, isAxesHelper: true, toJSON: () => ({ object: { type: 'AxesHelper' } }) })),
