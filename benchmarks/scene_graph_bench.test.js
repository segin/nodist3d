...
jest.mock('three', () => {
    ...
    const mockMesh = {
        ...
        material: {
            emissive: { setHex: jest.fn(), clone: jest.fn(() => ({ setHex: jest.fn() })) },
            color: { getHex: jest.fn(), setHex: jest.fn(), clone: jest.fn() },
            clone: jest.fn(() => ({ emissive: { setHex: jest.fn(), clone: jest.fn() }, color: { getHex: jest.fn(), clone: jest.fn() } }))
        },
        geometry: {
            clone: jest.fn(),
            dispose: jest.fn(),
    ...
    return {
        ...
        MeshLambertMaterial: jest.fn(() => ({
            ...
            color: { getHex: jest.fn(), setHex: jest.fn(), clone: jest.fn() },
            clone: jest.fn(() => ({ emissive: { setHex: jest.fn() }, color: { getHex: jest.fn() } }))
        })),
...
describe('SceneGraph Performance', () => {
    ...
    beforeEach(() => {
        // Setup JSDOM
        dom = new JSDOM('<!DOCTYPE html><html><body><div id="scene-graph"></div></body></html>', {
            url: 'http://localhost/',
