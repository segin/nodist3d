...
const IGNORE_DIRS = ['node_modules', '.git', 'dist', 'build', 'coverage', 'scripts', 'reference', 'verification'];

// Simple complexity proxy: count branching/control keywords
const COMPLEXITY_REGEX = /\b(if|else|while|for|case|catch|throw|return|&&|\|\||\?)\b/g;
...
// Top 10 Complexity
const topComplexity = [...results].sort((a, b) => b.complexity - a.complexity).slice(0, 10);
