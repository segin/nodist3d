...
const PATTERNS = [
    ...
    { name: 'AWS Secret Key', regex: /wJalrXUtnFEMI\/K7MDENG\/bPxRfiCYEXAMPLEKEY/g, severity: 'HIGH' }, // Example pattern
    { name: 'Google API Key', regex: /AIza[0-9A-Za-z\\-_]{35}/g, severity: 'HIGH' },
    { name: 'Generic Secret Assignment', regex: /(?:api_key|access_token|secret_key|password)\s*[:=]\s*['"]([a-zA-Z0-9_\-]{8,})['"]/gi, severity: 'MEDIUM' },
    { name: 'Private Key Block', regex: /-----BEGIN PRIVATE KEY-----/g, severity: 'CRITICAL' },
];

// Exclude these
const IGNORE_DIRS = ['node_modules', '.git', 'dist', 'build', 'coverage', 'scripts'];
const IGNORE_FILES = ['package-lock.json', 'pnpm-lock.yaml', 'audit_secrets.js', 'audit_metrics.js'];
