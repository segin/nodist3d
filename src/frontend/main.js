...
export class App {
    ...
    updatePropertiesPanel(object) {
        ...
        // Add rotation controls (in degrees)
    ...
    addGeometryProperties(object) {
        ...
        // ... (Other geometry types simplified for brevity if needed, but I should include them if possible)
        // I will include the rest
    ...
    getGeometryParameters(geometry) {
        ...
        switch (type) {
            case 'BoxGeometry':
                return { width: params.width || 1, height: params.height || 1, depth: params.depth || 1 };
            case 'SphereGeometry':
                return { radius: params.radius || 0.5, widthSegments: params.widthSegments || 32, heightSegments: params.heightSegments || 32 };
            case 'CylinderGeometry':
                return { radiusTop: params.radiusTop || 0.5, radiusBottom: params.radiusBottom || 0.5, height: params.height || 1 };
            case 'ConeGeometry':
                return { radius: params.radius || 0.5, height: params.height || 1 };
            case 'TorusGeometry':
                return { radius: params.radius || 0.4, tube: params.tube || 0.2 };
            case 'PlaneGeometry':
                return { width: params.width || 2, height: params.height || 2 };
            default:
                return {};
        }
    }
    ...
    updateSceneGraph() {
        ...
        // Restore focus
        if (focusedIndex >= 0 && this.objectsList.children.length > 0) {
            if (focusedIndex >= this.objectsList.children.length) {
                focusedIndex = this.objectsList.children.length - 1;
            }
            // @ts-ignore
            this.objectsList.children[focusedIndex].focus();
        }
    }
